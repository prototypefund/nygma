...
class bytestring_istream {
 ...
 public:
  ...
  explicit constexpr bytestring_istream(
      std::byte const* data, std::size_t const size, std::size_t const offset )
  ...
  template <
      typename T,
      std::enable_if_t<
          std::is_integral_v<T> && ( sizeof( T ) == 2 || sizeof( T ) == 4 || sizeof( T ) == 8 ),
          int> = 0>
